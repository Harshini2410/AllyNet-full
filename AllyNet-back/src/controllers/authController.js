...
const getUserStats = asyncHandler(async (req, res) => {
  ...
  // Count: Helped - count of accepted responses where user is helper
  // Use aggregation to count individual responses, not documents
  const helpedResult = await HelpRequest.aggregate([
    { $unwind: '$responses' },
    {
      $match: {
        'responses.helper': new mongoose.Types.ObjectId(userId),
        'responses.status': 'accepted'
      }
    },
    { $count: 'count' }
  ]);
  const helpedCount = helpedResult.length > 0 ? helpedResult[0].count : 0;

  // Count: Requested - help requests created by user
  const requestedCount = await HelpRequest.countDocuments({
    user: userId
  });

  // Count: SOS Alerts - emergencies created by user (SOS requested)
  const sosCount = await Emergency.countDocuments({
    user: userId
  });
...
const addEmergencyContact = asyncHandler(async (req, res) => {
  ...
  const existingContact = user.emergencyContacts?.find(
    contact => contact.phone.replace(/\s+/g, '') === phoneNormalized
  );
