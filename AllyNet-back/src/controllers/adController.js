...
const createAd = asyncHandler(async (req, res) => {
  const { title, description, image, category, businessName, businessType, adType, radiusKm, location, contact, expiresAt } = req.body;
  ...
  const ad = await adService.createAd(
    {
      ...
      businessType,
      adType: adType || 'local',
      radiusKm: adType === 'local' ? (radiusKm || 10) : undefined,
      location,
      contact,
      expiresAt
    },
