...
const getPendingEmergenciesForHelper = async (helperId) => {
  try {
    ...
    // STRICT QUERY: Only 'active' status (explicitly exclude all other statuses)
    const emergencies = await Emergency.find({
      status: 'active', // ONLY 'active' - excludes 'responding', 'resolved', 'cancelled'
    ...
    const pendingEmergencies = emergencies.filter(emergency => {
      // VALIDATION 1: Status must be EXACTLY 'active'
      if (emergency.status !== 'active') {
        return false; // Reject non-active emergencies
      }
