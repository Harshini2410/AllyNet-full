...
const createAd = async (adData, userId) => {
  ...
  const ad = await Ad.create({
    ...
    adType: adData.adType || 'local',
    radiusKm: adData.adType === 'local' ? (adData.radiusKm || 10) : undefined,
    location: {
 ...
 * Returns all global ads and local ads (no distance filtering for explore page)
...
const updateAd = async (adId, userId, updateData) => {
  ...
  const allowedFields = ['title', 'description', 'image', 'category', 'businessName', 'businessType', 'adType', 'radiusKm', 'location', 'contact', 'status'];
  ...
  // If adType is changed to global, remove radiusKm requirement
  if (ad.adType === 'global') {
    ad.radiusKm = undefined;
  } else if (ad.adType === 'local' && !ad.radiusKm) {
    // If changed to local and no radius provided, set default
    ad.radiusKm = updateData.radiusKm || 10;
  }
