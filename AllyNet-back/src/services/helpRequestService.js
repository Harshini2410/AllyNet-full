...
const createHelpRequest = async (requestData, userId) => {
  ...
  const helpRequest = await HelpRequest.create({
    ...
    radiusKm: requestData.radiusKm || 10, // User-selected radius (5, 10, 15, 20 km)
    budget: {
      amount: budgetAmount,
      currency: 'USD',
      negotiable: true
 ...
 * Filters requests by their individual radiusKm (each request has its own search radius)
 * @param {Number} latitude - Helper's latitude
 * @param {Number} longitude - Helper's longitude
 * @param {Number} radiusMeters - Maximum search radius in meters (default: 20000 = 20km)
 ...
 */
const getNearbyHelpRequests = async (latitude, longitude, radiusMeters = 20000, limit = 50, userId = null) => {
  // Get all nearby requests within max radius (20km)
  // We'll filter by each request's radiusKm below
  const allRequests = await HelpRequest.findNearbyOpen(latitude, longitude, radiusMeters, limit * 2);
  
  // Filter requests: helper must be within each request's radiusKm
  // Calculate distance for each request and check if within request's radiusKm
  const R = 6371; // Earth's radius in km
  const filteredRequests = [];
  
  for (const request of allRequests) {
    // Skip if created by current user
    if (userId && request.user._id.toString() === userId.toString()) {
      continue;
    }
    
    // Calculate distance from helper to request location
    const dLat = (request.location.latitude - latitude) * Math.PI / 180;
    const dLon = (request.location.longitude - longitude) * Math.PI / 180;
    const a = 
      Math.sin(dLat / 2) * Math.sin(dLat / 2) +
      Math.cos(latitude * Math.PI / 180) * Math.cos(request.location.latitude * Math.PI / 180) *
      Math.sin(dLon / 2) * Math.sin(dLon / 2);
    const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
    const distanceKm = R * c;
    
    // Check if helper is within request's chosen radiusKm
    if (distanceKm <= (request.radiusKm || 10)) {
