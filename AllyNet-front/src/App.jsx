  ...
  useEffect(() => {
    const restoreEmergencyAndNotifications = async () => {
      ...
      try {
        ...
        // This prevents showing stale notifications from previous sessions
        emergencyStore.clearNotifications();
        
        // Check if we've already restored notifications in this session (prevent showing on every refresh)
        const lastRestoreTime = localStorage.getItem('allynet-last-notification-restore');
        const now = Date.now();
        const RESTORE_COOLDOWN = 30000; // 30 seconds cooldown between restores
        
        if (lastRestoreTime && (now - parseInt(lastRestoreTime, 10)) < RESTORE_COOLDOWN) {
          // Recently restored - skip to prevent showing notifications on every refresh
          return;
        }
        
        // Add a small delay to prevent race conditions with state updates
        ...
        if (isCreator) {
          ...
          localStorage.setItem('allynet-last-notification-restore', now.toString());
          return;
        }
        ...
        if (!pendingResponse?.success || !Array.isArray(pendingResponse?.data)) {
          emergencyStore.clearNotifications(); // Ensure notifications are cleared
          localStorage.setItem('allynet-last-notification-restore', now.toString());
          return;
        }
        ...
        // ADDITIONAL CHECK: Verify emergency is still active before showing
        if (validEmergencies.length > 0) {
          const mostRecent = validEmergencies[0]; // Already sorted by createdAt desc
          
          // FINAL VERIFICATION: Double-check emergency status before showing
          // This prevents showing notifications for emergencies that were resolved between API call and now
          if (mostRecent.status === 'active' || mostRecent.status === 'responding') {
            // Additional safety: Verify user is not the creator (double-check)
            const emergencyUserId = mostRecent.user?._id?.toString() || mostRecent.user?.toString() || mostRecent.user;
            const currentUserId = authState.user?._id?.toString();
            
            if (!currentUserId || !emergencyUserId || currentUserId.toString() !== emergencyUserId.toString()) {
              // User is not the creator - safe to show notification
              emergencyStore.showNearbyEmergency(mostRecent);
              localStorage.setItem('allynet-last-notification-restore', now.toString());
            } else {
              // User is the creator - don't show notification
              emergencyStore.clearNotifications();
              localStorage.setItem('allynet-last-notification-restore', now.toString());
            }
          } else {
            // Emergency is not active/responding - don't show
            emergencyStore.clearNotifications();
            localStorage.setItem('allynet-last-notification-restore', now.toString());
          }
        } else {
          // No valid emergencies - ensure notifications are cleared
          emergencyStore.clearNotifications();
          localStorage.setItem('allynet-last-notification-restore', now.toString());
        }
      } catch (error) {
