...
import { Shield, Heart, MapPin, Info, ArrowRight } from 'lucide-react';
    ...
    <AppShell>
      ...
      <div className="space-y-6 pt-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Card className="p-4 flex flex-col gap-3 hover:bg-white dark:hover:bg-charcoal-700 transition-colors cursor-pointer group">
            ...
            <div>
              <h4 className="font-semibold text-sm text-charcoal-500 dark:text-sand-50">Safe Route</h4>
              <p className="text-[11px] text-charcoal-300 dark:text-sand-400">Community verified paths</p>
            </div>
          </Card>
        </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg text-charcoal-500 dark:text-sand-50">Nearby Community</h3>
            <button className="text-sage-600 dark:text-sage-400 text-sm font-medium">View all</button>
          </div>
          
          <div className="space-y-3">
            {[1, 2].map((i) => (
              <Card key={i} className="p-4 flex gap-4 items-center">
                <div className="w-12 h-12 rounded-2xl bg-sand-200 dark:bg-charcoal-700 shrink-0" />
                <div className="flex-1">
                  <h4 className="text-sm font-semibold text-charcoal-500 dark:text-sand-50">Community Update</h4>
                  <p className="text-xs text-charcoal-300 dark:text-sand-400 line-clamp-1">Heavy rain expected in the downtown area. Stay safe!</p>
                </div>
                <span className="text-[10px] text-charcoal-200 dark:text-sand-500">2m ago</span>
              </Card>
            ))}
          </div>
        </section>
        ...
        <section className="pt-8 pb-12">
          <div className="p-6 bg-sand-200/50 dark:bg-charcoal-800/50 rounded-[2.5rem] border border-sand-200 dark:border-charcoal-700">
            ...
            <button className="mt-6 flex items-center gap-2 text-xs font-bold text-sage-600 dark:text-sage-400">
    ...
    const restoreEmergencyAndNotifications = async () => {
      ...
      try {
        ...
        const validEmergencies = pendingEmergencies.filter(emergency => {
          ...
          // VALIDATION 2: Status must be EXACTLY 'active' (not responding, resolved, cancelled)
