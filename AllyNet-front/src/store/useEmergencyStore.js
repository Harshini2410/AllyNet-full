...
export const useEmergencyStore = create((set) => ({
  ...
  showNearbyEmergency: (emergency) => set((state) => {
    ...
    if (!emergency) {
      return { ...state, showEmergencyNotification: false, nearbyEmergency: null }; // No emergency - clear notification
    }
    ...
    if (emergency.status !== 'active' && emergency.status !== 'responding') {
      return { ...state, showEmergencyNotification: false, nearbyEmergency: null }; // Not active or responding - clear notification
    }
    
    // VALIDATION 2: Emergency must have an ID
    const emergencyId = emergency._id || emergency.id;
    if (!emergencyId) {
      return { ...state, showEmergencyNotification: false, nearbyEmergency: null }; // No ID - clear notification
    }
    ...
    if (helperEmergencyId && emergencyId && helperEmergencyId.toString() === emergencyId.toString()) {
      return { ...state, showEmergencyNotification: false, nearbyEmergency: null }; // User is already helping this emergency - clear notification
    }
    
    // VALIDATION 4: Don't show if user has an active emergency (they are creator)
    if (state.emergencyId && state.isActive) {
      return { ...state, showEmergencyNotification: false, nearbyEmergency: null }; // User has active emergency - clear notification
    }
