...
const AppShell = ({ children }) => {
  ...
  const globalActiveTab = useTabStore((state) => state.activeTab);
  const setGlobalActiveTab = useTabStore((state) => state.setActiveTab);
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative" ref={userMenuRef}>
            <button
              ...
              className="w-10 h-10 rounded-full bg-sage-200 dark:bg-charcoal-700 border-2 border-white dark:border-charcoal-800 shadow-sm flex items-center justify-center text-sage-700 dark:text-sage-300 font-bold hover:bg-sage-300 dark:hover:bg-charcoal-600 transition-colors cursor-pointer"
            >
              {initials}
            </button>
            ...
            <AnimatePresence>
              {isUserMenuOpen && (
                <motion.div
                  ...
                  className="absolute right-0 top-12 w-48 bg-white dark:bg-charcoal-800 rounded-lg shadow-lg border border-sand-200 dark:border-charcoal-700 overflow-hidden z-50"
                >
                  <div className="py-1">
                    <button
                      onClick={handleProfileClick}
                      className="w-full px-4 py-3 flex items-center gap-3 text-left text-sm text-charcoal-500 dark:text-sand-50 hover:bg-sand-50 dark:hover:bg-charcoal-700 transition-colors"
                    >
                      <User size={18} className="text-sage-600 dark:text-sage-400" />
                      <span>Profile</span>
                    </button>
                    <button
                      ...
                      className="w-full px-4 py-3 flex items-center gap-3 text-left text-sm text-charcoal-500 dark:text-sand-50 hover:bg-sand-50 dark:hover:bg-charcoal-700 transition-colors"
                    >
                      <LogOut size={18} className="text-coral-600 dark:text-coral-400" />
                      <span>Logout</span>
                    </button>
                  </div>
                </motion.div>
