  ...
  useEffect(() => {
    if (editData && isOpen) {
      setFormData({
        ...
        adType: editData.adType || 'local',
        radiusKm: editData.radiusKm || 10,
        location: {
    ...
    try {
      ...
      if (editData && editData._id) {
        ...
        response = await adApi.updateAd(editData._id, {
          ...
          radiusKm: formData.adType === 'local' ? formData.radiusKm : undefined,
          location: {
            latitude,
            longitude,
            ...
            <form onSubmit={handleSubmit} className="space-y-6 pb-8">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => setFormData({ ...formData, adType: 'local', radiusKm: 10 })}
                    className={cn(
                      'flex-1 px-4 py-3 rounded-xl text-sm font-bold transition-all',
                      formData.adType === 'local'
                        ? 'bg-sage-500 text-white'
                        : 'bg-white dark:bg-charcoal-700 text-charcoal-300 dark:text-sand-400 border border-sand-200 dark:border-charcoal-600'
                    )}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setFormData({ ...formData, adType: 'global' })}
                    className={cn(
                      'flex-1 px-4 py-3 rounded-xl text-sm font-bold transition-all',
                      formData.adType === 'global'
                        ? 'bg-sage-500 text-white'
                        : 'bg-white dark:bg-charcoal-700 text-charcoal-300 dark:text-sand-400 border border-sand-200 dark:border-charcoal-600'
                    )}
                  >
                ...
                <p className="mt-2 text-xs text-charcoal-400 dark:text-sand-400">
                  {formData.adType === 'local' 
                    ? 'Local ads are shown to users within your selected radius'
                    : 'Global ads are shown to all users regardless of location'}
                </p>
              </div>

              {/* Radius Selector (only for local ads) */}
              {formData.adType === 'local' && (
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-sage-600 dark:text-sage-400 block mb-2">
                    Reach Radius *
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {[5, 10, 15, 20].map((radius) => (
                      <button
                        key={radius}
                        type="button"
                        onClick={() => setFormData({ ...formData, radiusKm: radius })}
                        className={cn(
                          'px-4 py-3 rounded-xl text-sm font-bold transition-all',
                          formData.radiusKm === radius
                            ? 'bg-sage-500 text-white'
                            : 'bg-white dark:bg-charcoal-700 text-charcoal-300 dark:text-sand-400 border border-sand-200 dark:border-charcoal-600'
                        )}
                      >
                  ...
                  <p className="mt-2 text-xs text-charcoal-400 dark:text-sand-400">
                    Your ad will be visible to users within {formData.radiusKm} km radius
                  </p>
                </div>
              )}
