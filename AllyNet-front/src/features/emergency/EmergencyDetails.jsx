...
const EmergencyDetails = () => {
  ...
  // Poll for updates every 5 seconds to get updated helper count (only if user is a helper or has accepted)
  useEffect(() => {
    if (!id || (!isHelper && !acceptSuccess)) return;
  ...
  const getLocationText = (location) => {
    ...
    if (location.latitude !== undefined && location.longitude !== undefined) {
      // Display as: Lat: X.XXXXXX, Lon: Y.YYYYYY
  ...
  const shouldShowAcceptButton = () => {
    if (!emergency || !currentUser) return false;
    // Allow accepting if status is 'active' or 'responding' (supports multiple helpers)
    return (emergency.status === 'active' || emergency.status === 'responding') && !isCreator() && !isHelper && !acceptSuccess;
  };
  ...
  return (
    <div className="min-h-screen bg-sand-100 dark:bg-charcoal-900 pb-24">
      ...
      <div className="px-6 py-6 space-y-4">
        ...
        <Card 
          className="p-6 cursor-pointer hover:bg-sand-50 dark:hover:bg-charcoal-700/50 transition-colors"
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  {emergency?.respondingHelpers && emergency.respondingHelpers.length > 0 && (
                    <p className="text-xs text-sage-600 dark:text-sage-400 mt-2 font-medium">
                      {emergency.respondingHelpers.length} Helper{emergency.respondingHelpers.length !== 1 ? 's' : ''} responding
                    </p>
                  )}
                </div>
              </div>
      ...
      {/* Map Modal - Shows emergency location */}
      <EmergencyMapModal
        isOpen={showMap}
