...
const SOSOverlay = ({ isOpen, onClose }) => {
  const { radius, setRadius } = useEmergencyStore();
  ...
  const emergencyTypes = [
    ...
    { id: 'harassment', label: 'Harassment', icon: EyeOff, color: 'text-purple-500', bg: 'bg-purple-50' },
  ...
  const handleTrigger = async () => {
    ...
    try {
      ...
      // Call API to create emergency (API is source of truth)
      const response = await emergencyApi.createEmergency({
        ...
        type: selectedType,
        avoidRadiusKm: radius / 1000, // Convert meters to km
        priority: 'high',
        severity: 5,
        anonymousMode: anonymousMode,
        silentMode: false, // Keep silentMode false for now
        fakeCallAlert: fakeCallAlert // New feature for fake call alerts
      });
        ...
        <>
          ...
          >
            ...
            <div className="space-y-8">
              ...
              {/* Avoidance Radius Selection */}
              <div>
                <div className="flex justify-between items-center mb-4">
                  <p className="text-xs font-bold uppercase tracking-widest text-sage-600">Avoidance Radius</p>
                ...
                <input
                  ...
                  className="w-full h-2 bg-sand-200 rounded-lg appearance-none cursor-pointer accent-sage-500"
                />
                <div className="flex justify-between text-[10px] text-charcoal-200 mt-2 font-bold uppercase">
                  <span>0.5km</span>
                  <span>2km</span>
                </div>
              </div>
