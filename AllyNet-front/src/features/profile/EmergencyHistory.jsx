...
import { ArrowLeft, Shield, MapPin, Clock, CheckCircle, XCircle, AlertCircle, Trash2 } from 'lucide-react';
...
const EmergencyHistory = () => {
  ...
  const handleDelete = async (emergencyId, e) => {
    e.stopPropagation(); // Prevent opening modal
    if (!window.confirm('Are you sure you want to delete this emergency from your history?')) {
      return;
    }

    setDeletingId(emergencyId);
    try {
      const response = await emergencyApi.deleteEmergency(emergencyId);
      if (response?.success) {
      ...
      alert(err.message || 'Failed to delete emergency. Please try again.');
    } finally {
      setDeletingId(null);
    }
  };
            ...
            <div className="space-y-3">
              {sosRequested.map((emergency) => (
                <EmergencyCard
                  ...
                  onDelete={(e) => handleDelete(emergency._id, e)}
                  isDeleting={deletingId === emergency._id}
                />
              ))}
            </div>
          )}
...
const EmergencyCard = ({ emergency, role, onClick, onDelete, isDeleting }) => {
  const formatTime = (dateString) => {
    if (!dateString) return 'N/A';
    const date = new Date(dateString);
    return date.toLocaleString('en-US', {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        ...
        {onDelete && (emergency.status === 'resolved' || emergency.status === 'cancelled') && (
          <button
            onClick={onDelete}
            disabled={isDeleting}
            className="p-2 hover:bg-coral-50 dark:hover:bg-coral-900/20 rounded-full transition-colors disabled:opacity-50"
            title="Delete emergency"
          >
            {isDeleting ? (
              <div className="w-4 h-4 border-2 border-coral-600 border-t-transparent rounded-full animate-spin" />
            ) : (
